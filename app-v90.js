...
const cloneZikirDefaults=()=>({
  enabled:C.zikirMode?.enabled!==false,
  step1Seconds:Number(C.zikirMode?.step1Seconds||45),
  step2Seconds:Number(C.zikirMode?.step2Seconds||75),
  step3Seconds:Number(C.zikirMode?.step3Seconds||90),
  closingSeconds:Number(C.zikirMode?.closingSeconds||10),
  prayers:{...(C.zikirMode?.prayers||{})}
});
...
async function loadPrayerConfig(){
  ...
  const results=await Promise.allSettled([
    ...
    querySheet(sheets.zikirMode||"Mode Zikir")
  ]);
