...
const E={
  ...
  prayerName:$("prayerModePrayer"),prayerCountdown:$("prayerModeCountdown"),prayerMessage:$("prayerModeMessage"),prayerDuaCard:$("prayerDuaCard"),prayerRamadanSpeaker:$("prayerRamadanSpeaker"),prayerRamadanSpeakerName:$("prayerRamadanSpeakerName"),prayerAdviceItem1:$("prayerAdviceItem1"),prayerAdviceItem2:$("prayerAdviceItem2"),prayerAdviceItem3:$("prayerAdviceItem3"),prayerAdvice1:$("prayerAdvice1"),prayerAdvice2:$("prayerAdvice2"),prayerAdvice3:$("prayerAdvice3"),
...
function applyRamadanRows(rows){
  ...
  ramadanSettings={
    ...
    speakerName:String(configRow["Nama Penceramah"]||C.prayerMode?.ramadanDefaults?.speakerName||"Penceramah akan diumumkan").trim(),
...
function ramadanEntryFor(now,{allowPreviewFallback=false}={}){
  ...
  if(night){
    const exact=ramadanSchedule.find(item=>item.night===night);
    if(exact)return exact;
    return{night,dateKey:"",speakerName:"",theme:""};
  }
  if(allowPreviewFallback){
    return ramadanSchedule.find(item=>item.speakerName)||ramadanSchedule[0]||null;
  }
  return null;
}
...
function getPrayerModeState(now,times){
  ...
  for(const [name,start] of events){
    ...
    const postAzanStart=t;
    const duaMinutes=minutesValue(C.prayerMode?.postAzanDuaMinutes,3);
    if(name==="Jumat"){
      end=t+cfg.khutbahMinutes*60000;
      const duaEnd=Math.min(end,postAzanStart+duaMinutes*60000);
      if(nowMs>=postAzanStart&&nowMs<divaEnd)return phaseState("dua",name,postAzanStart,end);
      if(nowMs>=duaEnd&&nowMs<end)return phaseState("khutbah",name,postAzanStart,end);
      t=end;
    }else{
      end=t+cfg.iqamahMinutes*60000;
      const duaEnd=Math.min(end,postAzanStart+duaMinutes*60000);
      if(nowMs>=postAzanStart&&nowMs<divaEnd)return phaseState("dua",name,postAzanStart,end);
      if(nowMs>=duaEnd&&nowMs<end)return phaseState("iqamah",name,postAzanStart,end);
      t=end;
    }
...
function renderPrayerMode(state,now){
  ...
  E.prayerTitle.textContent=titles[state.phase]||state.title||"";
  E.prayerName.textContent=state.phase==="ramadanSermon"?"MENJELANG TARAWIH & WITIR":state.phase==="tarawih"?"RAMADAN":String(state.prayerName||"").toUpperCase();
  ...
  if(E.prayerMessage)E.prayerMessage.textContent=state.phase==="dua"?"Hitungan mundur tetap menuju "+(state.prayerName==="Jumat"?"akhir khutbah Jumat":"iqamah")+".":state.message;
...
function getPrayerPreviewState(now){
  ...
  if(phase==="ramadansermon")phase="ramadanSermon";
  if(prayerName==="Jumat"&&phase==="iqamah")phase="khutbah";
  if(!["azan","dua","iqamah","khutbah","prayer","ramadanSermon","tarawih"].includes(phase))phase=prayerName==="Jumat"?"khutbah":"iqamah";
  ...
  const duration=
    ...
    phase==="dua"?minutesValue(prayerName==="Jumat"?cfg.khutbahMinutes:cfg.iqamahMinutes,prayerName==="Jumat"?30:10):
