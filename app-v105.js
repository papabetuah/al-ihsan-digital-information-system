...
let prayerModeActive=false,prayerModeSignature="",prayerWatchdogTimerId=0;
let runtimeStartedAtMs=Date.now(),lastRuntimeRefreshAttemptMs=0,lastPrayerReturnScene=C.rotation?.startScene||"dashboard";
const RUNTIME_SAFE_REFRESH_MS=4*60*60*1000;
...
function render(){
  try{
    ...
    // PRIORITAS MUTLAK: Mode Sholat diperiksa sebelum elemen dashboard umum.
    // Error pada jam, tanggal, CMS, atau overlay umum tidak boleh mencegah
    // layar masuk ke Mode Sholat ketika waktunya tiba.
...
function prayerPriorityWatchdog(){
  try{
    const now=new Date();
    const{times}=getDailyTimes(now);

    // Jalur independen dari render utama. Walau overlay/CMS mengalami error,
    // pemeriksaan Mode Sholat tetap berjalan setiap detik.
    updatePrayerMode(now,times);

    // Yodeck/Android WebView dapat mempertahankan halaman lama berjam-jam.
    // Refresh berkala hanya dilakukan di luar Mode Sholat agar runtime terbaru
    // terambil tanpa memotong sesi ibadah yang sedang berlangsung.
    const forced=previewQuery.get("scene");
    if(!prayerModeActive&&!["prayer","zikir"].includes(forced)&&now.getTime()-runtimeStartedAtMs>=RUNTIME_SAFE_REFRESH_MS){
      if(now.getTime()-lastRuntimeRefreshAttemptMs>60000){
        lastRuntimeRefreshAttemptMs=now.getTime();
        const url=new URL(location.href);
        url.searchParams.set("_runtime",String(Math.floor(now.getTime()/RUNTIME_SAFE_REFRESH_MS)));
        location.replace(url.toString());
      }
    }
  }catch(error){
    console.error("Prayer priority watchdog failed",error);
    if(E.status)E.status.style.background="#ffb158";
  }
}

function startPrayerPriorityWatchdog(){
  clearInterval(prayerWatchdogTimerId);
  prayerPriorityWatchdog();
  prayerWatchdogTimerId=setInterval(prayerPriorityWatchdog,1000);
}
...
function init(){
  ...
  scheduleNextTick();startPrayerPriorityWatchdog();scheduleSceneRotation();
  ...
  document.addEventListener("visibilitychange",()=>{if(!document.hidden){fit();prayerPriorityWatchdog();render();loadCms();loadPrayerConfig()}});
  addEventListener("pageshow",()=>{prayerPriorityWatchdog();render()},{passive:true});
  addEventListener("focus",()=>{prayerPriorityWatchdog()},{passive:true});
  addEventListener("online",()=>{prayerPriorityWatchdog();loadPrayerConfig()},{passive:true});
