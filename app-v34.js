...
const E={stage:$("stage"),donation:$("donationScene"),donationUnderlay:$("donationUnderlay"),date:$("liveDate"),clockHM:$("clockHourMinute"),clockSec:$("clockSecond"),countdown:$("countdown"),next:$("nextPrayer"),shubuh:$("timeShubuh"),terbit:$("timeTerbit"),dzuhur:$("timeDzuhur"),ashar:$("timeAshar"),maghrib:$("timeMaghrib"),isya:$("timeIsya"),status:$("statusDot")};
...
async function loadDonationUnderlay(){
  ...
  try{
    ...
    const urls=Array.from({length:count},(_,i)=>
      `./assets/donation-underlay-v30/part-${String(i+1).padStart(2,"0")}.b64?v=underlay-final-20260924-v30-donation-underlay`
    );
...
function setScene(scene){
  currentScene=scene==="donation"?"donation":"dashboard";
...
function sceneDuration(scene){
  const R=C.rotation||{};
  return scene==="donation"?(R.donationMs||30000):(R.dashboardMs||45000);
}
...
function scheduleSceneRotation(){
  ...
  if(forced==="dashboard"||forced==="donation"){
    setScene(forced);
    return;
  }
  sceneTimerId=setTimeout(()=>{
    setScene(currentScene==="dashboard"?"donation":"dashboard");
...
function init(){
  fit();
  loadDonationUnderlay();
  const params=new URLSearchParams(location.search);
  const forced=params.get("scene");
  setScene(forced==="dashboard"||forced==="donation"?forced:(C.rotation?.startScene||"dashboard"));
  render();
  scheduleNextTick();
  scheduleSceneRotation();
