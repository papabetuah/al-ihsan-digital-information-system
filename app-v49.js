...
function renderAnnouncement(index=announcementIndex){
  const fallback={
    Prioritas:"NORMAL",Kategori:"INFORMASI JAMAAH",Judul:"Pengumuman Masjid Al Ihsan",
    "Isi Pengumuman":"Informasi kegiatan, kajian, gotong royong, pelayanan jamaah, dan pengumuman penting akan tampil otomatis di halaman ini.",
    "Waktu Kegiatan":"Dikelola melalui Google Sheet oleh pengurus",Lokasi:"Masjid Al Ihsan Kapuih"
  };
  ...
  E.annPriority.textContent=priority==="DARURAT"?"PENGUMUMAN DARURAT":priority==="UTAMA"?"PENGUMUMAN UTAMA":String(item.Kategori||"INFORMASI JAMAAH").toUpperCase();
  ...
  E.annTitle.textContent=String(item.Judul||"Pengumuman Masjid Al Ihsan");
  E.annBody.textContent=String(item["Isi Pengumuman"]||"");
  E.annTime.textContent=String(item["Waktu Kegiatan"]||item.Kategori||"Informasi Jamaah");
  E.annLocation.textContent=String(item.Lokasi||"Masjid Al Ihsan Kapuih");
  E.annPager.textContent=`${announcementIndex+1} / ${list.length}`;
...
function renderAgenda(){
  ...
  if(!list.length){
    const empty=document.createElement("div");empty.className="announcement-agenda-empty";empty.textContent="Belum ada agenda aktif.";
    E.annAgenda.appendChild(empty);return;
  }
